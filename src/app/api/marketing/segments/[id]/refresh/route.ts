...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const smsEligibleCount = eligible.filter((u: any) => { const p = u.fields?.phone?.stringValue; return p && p.trim().length > 5; }).length;
  ...
  await fetch(
    ...
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fields: { memberCount: { integerValue: count }, smsEligibleCount: { integerValue: smsEligibleCount }, lastRefreshedAt: { stringValue: now } } }) }
  );
