...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await fetch(`${FIRESTORE_BASE}/support_tickets/${ticketId}/messages`, {
    ...
    body: JSON.stringify({ fields: { senderType: { stringValue: 'agent' }, senderName: { stringValue: agentName || 'Support Agent' }, body: { stringValue: body }, sentVia: { stringValue: 'app' }, sentAt: { stringValue: now } } }),
  });
  await fetch(
    ...
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fields: { status: { stringValue: 'pending_user' }, updatedAt: { stringValue: now }, lastMessage: { stringValue: body.slice(0, 100) } } }) }
  );
  ...
  if (ticketRes.ok) {
    ...
    if (senderEmail) {
      sendAgentReplyNotification({ ticketId, subject, replyBody: body, agentName: agentName || 'Support Agent', senderName: senderName || senderEmail, senderEmail }).catch(err => console.error('[email]', err));
    }
  }
