...
const MAX_BYTES = 50 * 1024 * 1024; // 50 MB (reduced from 500 MB — prevents DoS)
...
const PATH_MIME_RULES: { prefix: string; allowed: string[] }[] = [
  { prefix: 'profile-photos/', allowed: ALLOWED_MIME_TYPES.image },
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate path prefix per UID and enforce path→MIME rules
    ...
    // Enforce MIME type matches path type
    const pathRule = PATH_MIME_RULES.find(rule => storagePath.startsWith(rule.prefix));
    if (pathRule && !pathRule.allowed.includes(mimeType)) {
      return NextResponse.json({ error: `File type ${mimeType} not allowed for this path` }, { status: 415 });
    }
    ...
    console.error('[upload] Unhandled error:', err?.message ?? 'Unknown');
