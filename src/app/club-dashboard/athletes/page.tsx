...
import { Loader2, ArrowUpDown, ShieldCheck, Clock, ExternalLink, Search, MessageSquare } from 'lucide-react';
...
export default function SquadListPage() {
  ...
  const athleteIds = React.useMemo(() => {
    const ids = [...new Set((squadMembers || []).map(m => m.userId).filter(Boolean))];
    return ids;
  }, [squadMembers]);
  ...
  const openDM = React.useCallback((athleteId: string) => {
    if (!user?.uid) return;
    const convId = dmConvId(user.uid, athleteId);
      ...
      {isLoading ? (
        ...
        <div className="flex h-32 items-center justify-center text-center">
          <p className="text-muted-foreground font-bold uppercase text-[10px] tracking-widest">
            No squad members yet — invite athletes to join your club
          </p>
        </div>
      ) : (
        <>
